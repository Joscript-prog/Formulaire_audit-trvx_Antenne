...
// ============================================================
// ---------- VALIDATION : 12 PHOTOS MIN (6 lieux + 6 écrans) + POINTS PLACÉS SUR LE PLAN ----------
// Règles métier :
//   1. Au moins 3 points 4G et 3 points 5G doivent exister dans la section 3.
//   2. Chaque point doit avoir SES DEUX photos (lieu + copie d'écran),
//      ce qui garantit au minimum 12 photos (6 lieux + 6 écrans) pour 6 points.
//   3. Tous les points existants doivent être positionnés sur le plan d'évacuation,
//      SAUF si la case "Aucun plan d'évacuation disponible sur le site" est cochée.
function validateMeasurementPoints() {
    ...
    const points4G = container4G ? Array.from(container4G.querySelectorAll(".measure-point-group")) : [];
    const points5G = container5G ? Array.from(container5G.querySelectorAll(".measure-point-group")) : [];

    // 1) Nombre minimum de points par techno
    if (points4G.length < 3) {
        errors.push(`Il faut au moins 3 points de mesure 4G (actuellement : ${points4G.length}).`);
    }
    if (points5G.length < 3) {
        errors.push(`Il faut au moins 3 points de mesure 5G (actuellement : ${points5G.length}).`);
    }

    // 2) Comptage des photos : chaque point doit avoir photo lieu + photo écran
    let photoCount = 0;
    const missingPhotos = []; // pour message détaillé
    ...
    allPoints.forEach((group, idx) => {
        ...
        if (!hasLieu || !hasScreen) {
            ...
            missingPhotos.push(`Point ${tech} ${num} : ${missingParts.join(" + ")} manquante(s)`);
        }
    });

    // Règle principale : au moins 12 photos (6 points × 2 photos)
    if (photoCount < 12) {
        ...
        // On joint le détail des points incomplets pour aider l'utilisateur
        missingPhotos.slice(0, 6).forEach(m => errors.push("• " + m));
    }

    // 3) Positionnement sur le plan d'évacuation (sauf si la case "pas de plan" est cochée)
    ...
    if (!noPlan) {
        ...
        if (!planLoaded) {
            ...
            // Tous les points existants doivent être placés
            const placedIds = new Set(evacPoints.map(p => p.pointId));
...
function updateGenerateButtonState() {
    ...
    if (v.valid) {
        ...
        banner.innerHTML = `
            <div class="vb-title">✅ Audit complet — Génération autorisée</div>
            <div style="font-size:0.9em;">${v.photoCount} photos relevées${v.noPlan ? " — site sans plan d'évacuation (déclaré par le technicien)" : " — tous les points sont positionnés sur le plan"}.</div>
        `;
    } else {
